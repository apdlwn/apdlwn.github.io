...
* 최적화 알고리즘 구현 (기본 동작방식 + 알고리즘에서 사용한 함수 및 매개변수 값들에 대한 설명 필요)
    - ex) 유전 알고리즘에서 돌연변이는 어떤식으로 발생하고 어느정도의 확률로 발생하는지, 확률이 너무 높거나 낮으면 어떤 현상이 발생하는지

* 모수 값 추정 (추정된 모수 값이 적절한지 검증)

* 최적화 과정에 대한 분석 (최적화되는 과정에서 에러가 감소하는 경향 표현)

#### 알고리즘 구현

* 가정한 함수가 ``y=a^x`` 이므로 모수의 값을 추정하는 방법 말고, a의 값을 랜덤으로 여러개 만든 후 그 값들 중에서 가장 최적화된 값을 찾는 알고리즘을 구현해보았습니다.
...
//랜덤하게 설정된 a의 적합도는 모든 입력(5-16)에 대한 (a^x-원래data)^2 값을 모두 더한 후 나누기 12를 하는 것으로 계산함.
...
int main(){
    ...
    calculatefitness(fitness, genedata, randoma);
    calculatewheeltable(wheeltablesize, fitness);
    selection(selected, wheeltablesize, randoma);
    double selectedbinary[8]={0}; //선택된 값들의 2진수를 저장하는 배열
    crossover(selected, selectedbinary); //selectedbinary 배열에 교차 연산된 값들이 저장
    double mutate=0.125;  //돌연변이 확률 지정
    Mutate(mutate, selectedbinary);  //selectedbinary 배열에 돌연변이 연산된 값들도 저장
    binarytodecimal(selected, selectedbinary);
    calculatefitness(fitness, genedata, selected);
    printf("[Result]\n");
    for(int i=0; i<8; i++){
...
=> 결과를 보면 1.4~ 들과 1.3~ 의 적합도가 높은 것을 확인할 수 있습니다. 그러므로 a에 적합한 값은 1.3~1.4 인 것을 알 수 있습니다. 또한 처음 랜덤으로 만들어진 8개의 a를 보면 적합도가 높은 1.3~1.4는 4개인 반면에 Result 값들을 보면 5개인 것을 확인할 수 있습니다. 그러므로 두 번째 세대의 평균적합도가 향상된 것을 확인할 수 있습니다. 

각 a에 대한 그래프를 살펴보면

![image](https://user-images.githubusercontent.com/101469532/174243092-a37a62eb-288f-4090-95af-fd73ae349fe9.png)
